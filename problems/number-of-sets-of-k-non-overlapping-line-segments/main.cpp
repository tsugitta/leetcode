 ...
 public:
  int numberOfSets(int N, int K) {
    ...
    rep1(k, K) {
      ...
      rep1(i, N - 1) {
        // i 番目を右端とする辺を取る場合: dp[i-1][k]
        // 取らない場合: \sum_{j=0}^{i-1} dp[j][k-1]
        // k を固定とすれば累積和を持てば高速化できる
        sum += dp[i - 1][k - 1];
        dp[i][k] = sum + dp[i - 1][k];
      }
    }
  ...
  int numberOfSets2(int N, int K) {
    prepare_combination(N + K + 100);
    return c_m(N + K - 1, 2 * K).val;
  }
};
