...
struct MyTreeNode {
  ...
  MyTreeNode* left;
  MyTreeNode* right;
  MyTreeNode(int x) : val(x), left(NULL), right(NULL) {}
...
class Solution {
 public:
  int numOfWays(vector<int>& nums) {
    ...
    prepare_combination(N + 5);

    repf(i, 1, N) { root->add(nums[i]); }

    // 個数、通り
    auto dfs = [](auto self, MyTreeNode* node) -> P<ll, mint> {
      if (!node) {
        return {0, 1};
      }

      auto [lc, lp] = self(self, node->left);
      auto [rc, rp] = self(self, node->right);

      mint p = c_m(lc + rc, lc) * lp * rp;
      return {lc + rc + 1, p};
    };

    auto [_, res] = dfs(dfs, root);
